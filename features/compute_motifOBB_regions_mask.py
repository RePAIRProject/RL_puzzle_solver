...
def main(args):
    ...
    for puzzle in puzzles:
        ...
        for i in range(len(pieces)):
            for j in range(len(pieces)):
                ...
                if i == j:
                    ...
                    mask_i = cv2.resize(piece_i_on_canvas['lines_mask'],
                                        (ppars.comp_matrix_shape[0], ppars.comp_matrix_shape[1]))
                    # outside_borders_i = get_outside_borders(mask_i, borders_width=1)
                    for t in range(grid_size_rot):
                        piece_j_on_canvas = place_on_canvas(pieces[j], (center_pos, center_pos), ppars.canvas_size,
                                                            t * ppars.theta_step)
                        mask_j = cv2.resize(piece_j_on_canvas['lines_mask'],
                                            (ppars.comp_matrix_shape[0], ppars.comp_matrix_shape[1]))
                        # outside_borders_j = get_outside_borders(mask_j, bordpers_width=1)
                        overlap_lines = cv2.filter2D(piece_i_on_canvas['lines_mask'], -1,
                                                     piece_j_on_canvas['lines_mask'])
                        ...
                        thr_reg_map_comp_range = thr_reg_map_shape_uint[ppars.p_hs + 1:-(ppars.p_hs + 1),
                        ...
                        resized_shape = np.array(Image.fromarray(thr_reg_map_comp_range).resize(
                        ...
                        combo_comp_range = combo_uint[ppars.p_hs + 1:-(ppars.p_hs + 1),
                                           ppars.p_hs + 1:-(ppars.p_hs + 1)]
                        ...
                        resized_combo = np.array(Image.fromarray(combo_comp_range).resize(
                            (ppars.comp_matrix_shape[0], ppars.comp_matrix_shape[1]), Image.Resampling.NEAREST))
                        ...
                        resized_lines = np.array(Image.fromarray(binary_overlap_lines).resize(
                            (ppars.comp_matrix_shape[0], ppars.comp_matrix_shape[1]), Image.Resampling.NEAREST))
        ...
        if args.save_visualization is True:
            ...
            save_vis(RM_combo, pieces, ppars.theta_step, os.path.join(vis_folder,
                                                                      f'visualization_combo_{puzzle}_{grid_size_xy}x{grid_size_xy}x{grid_size_rot}x{len(pieces)}x{len(pieces)}'),
                     f"regions matrix {puzzle}", save_every=4, all_rotation=False)
            save_vis(RM_lines, pieces, ppars.theta_step, os.path.join(vis_folder,
                                                                      f'visualization_lines_{puzzle}_{grid_size_xy}x{grid_size_xy}x{grid_size_rot}x{len(pieces)}x{len(pieces)}'),
                     f"overlap {puzzle}", save_every=4, all_rotation=False)
            save_vis(RM_shapes, pieces, ppars.theta_step, os.path.join(vis_folder,
                                                                       f'visualization_shapes_{puzzle}_{grid_size_xy}x{grid_size_xy}x{grid_size_rot}x{len(pieces)}x{len(pieces)}'),
                     f"borders {puzzle}", save_every=4, all_rotation=False)
