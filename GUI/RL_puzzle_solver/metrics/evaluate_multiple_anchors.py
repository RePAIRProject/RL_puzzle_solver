...
def main(args):
    ...
    q_eval_folder = os.path.join(evaluation_folder, 'quantitative_evaluation')
    ql_eval_folder = os.path.join(evaluation_folder, 'qualitative_evaluation')
    ...
    for j in range(0, 64):
        ...
        eval_file_path = os.path.join(q_eval_folder, eval_file_json)
    ...
    best_correct_img, best_correct_idx, best_correct_pos, best_correct_val, conf_matrix_correct = get_best_anchor(im_ref, eval_cur_img[:,0], args.num_pieces, cfg.piece_size, best='max')
    best_neighbours_img, best_neighbours_idx, best_neighbours_pos, best_neighbours_val, conf_matrix_neighbour = get_best_anchor(im_ref, eval_cur_img[:,2], args.num_pieces, cfg.piece_size, best='max')
    best_pixel_img, best_pixel_idx, best_pixel_pos, best_pixel_val, conf_matrix_pixel = get_best_anchor(im_ref, eval_cur_img[:,3], args.num_pieces, cfg.piece_size, best='min')


    plt.figure(figsize=(32,32))
    plt.suptitle(f"{args.puzzle}", fontsize=40)
    ...
    solution_best_correct = plt.imread(os.path.join(ql_eval_folder, f"evaluated_solution_anchor{best_correct_idx}.jpg"))
    ...
    solution_best_neighbours = plt.imread(os.path.join(ql_eval_folder, f"evaluated_solution_anchor{best_neighbours_idx}.jpg"))
    ...
    solution_best_pixel = plt.imread(os.path.join(ql_eval_folder, f"evaluated_solution_anchor{best_pixel_idx}.jpg"))
    plt.imshow(solution_best_pixel)

    ### WORST
    best_correct_img, best_correct_idx, best_correct_pos, best_correct_val, _ = get_best_anchor(im_ref, eval_cur_img[:,0], args.num_pieces, cfg.piece_size, best='min')
    best_neighbours_img, best_neighbours_idx, best_neighbours_pos, best_neighbours_val, _ = get_best_anchor(im_ref, eval_cur_img[:,2], args.num_pieces, cfg.piece_size, best='min')
    best_pixel_img, best_pixel_idx, best_pixel_pos, best_pixel_val, _ = get_best_anchor(im_ref, eval_cur_img[:,3], args.num_pieces, cfg.piece_size, best='max')
    ...
    # confusion matrices
    plt.subplot(4, 4, 5)
    plt.title('Confusion Matrix (CORRECT)', fontsize=24)
    plt.imshow(conf_matrix_correct, cmap='jet')
    plt.colorbar()
    plt.subplot(4, 4, 9)
    plt.title('Confusion Matrix (NEIGHBOURS)', fontsize=24)
    plt.imshow(conf_matrix_neighbour, cmap='jet')
    plt.colorbar()
    plt.subplot(4, 4, 13)
    plt.title('Confusion Matrix (PIXEL)', fontsize=24)
    plt.imshow(conf_matrix_pixel, cmap='jet')
    plt.colorbar()
