...
import numpy as np
import cv2
from GUI.RL_puzzle_solver.solver.puzzle_utils import PuzzlePiece
import shapely
    ...
    def __init__(self, grid_parameters, piece_size):
        ...
        for b in range(len(axis_grid)):
            for g in range(len(axis_grid)):
                pieces_grid[g, b] = (zero_aligned_axis_grid[g] + canvas_alignment,
                                     zero_aligned_axis_grid[b] + canvas_alignment)
...
class PieceOnCanvas:
    ...
    def __init__(self, piece: PuzzlePiece, grid: PuzzleGrid, x: float, y: float, theta: float,
        ...
        if enabled_features is not None:
            ...
            if enabled_features['motives'] == True:
                self.motives_cube = np.zeros(
                    (grid.canvas_size, grid.canvas_size, piece.features.motives.num_of_classes))
        ...
        if enabled_features is not None:
            ...
            if enabled_features['lines'] == True:
                lines_mask = scipy.ndimage.rotate(piece.features.lines_mask, theta, reshape=False, mode='constant',
                                                  order=0, prefilter=False)
                lines_mask = cv2.morphologyEx(lines_mask, cv2.MORPH_CLOSE, closing_kernel)
            ## NEW MOTIF-BASED
            if enabled_features['motives'] == True:
                motives_cube = scipy.ndimage.rotate(piece.features.motives.motives_cube, theta, reshape=False,
