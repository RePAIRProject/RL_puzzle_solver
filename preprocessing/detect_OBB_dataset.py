...
def main(args):
    ...
    puzzle_folders = os.listdir(args.dataset)
    sorted_puzzle_folders = natsort.natsorted(puzzle_folders)
    #skip_done = params['skip_done']

    for puzzle_folder in sorted_puzzle_folders[52:]:

        imgs_folder = os.path.join(args.dataset, puzzle_folder, 'images')
        motifs_output = os.path.join(args.dataset, puzzle_folder)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-i', '--dataset', type=str, default='', help='images input folder')
