...
def main(args):
    ...
    for N in range(args.num_images):
        ...
        for j, piece in enumerate(pieces):
            ## save patch
            cv2.imwrite(os.path.join(pieces_single_folder, f"piece_{j:04d}.png"), piece['img'])
            cv2.imwrite(os.path.join(masks_single_folder, f"piece_{j:04d}.png"), piece['mask'])
            np.save(os.path.join(poly_single_folder, f"piece_{j:04d}"), piece['poly'])
            ...
            for i in range(args.num_lines):
                ...
                intersect = shapely.intersection(line, piece['poly'])  # points of intersection line with the piece
                if shapely.is_empty(intersect) == False: # we have intersection, so the line is important for this piece
                    if len(list(zip(*intersect.xy))) > 1: # two intersections meaning it crosses
                        pi1, pi2 = list(zip(*intersect.xy))
            ...
            print(f'done with piece_{j:04d}')

if __name__ == '__main__':

    parser = argparse.ArgumentParser(description='It creates `ni` images with `nl` of lines/segments')
    parser.add_argument('-nl', '--num_lines', type=int, default=1, help='min number of lines')
