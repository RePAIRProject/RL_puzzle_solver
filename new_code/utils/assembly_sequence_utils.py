...
def get_assembly_plan_snake(polygons_data, movement_delay=0.01, viz=False):
    ...
    while smaller_box_x + 0.005 < larger_box_x + larger_box_width:
        ...
        for vertex in all_vertices:
            x, y = vertex
            if (smaller_box_x <= x <= smaller_box_x + smaller_box_width) and (smaller_box_y <= y <= smaller_box_y + smaller_box_height):
        ...
        for vertex in vertices_inside_box:
            for polygon_id, polygon_data in polygons_data.items():
                if vertex in polygon_data['vertices']:
                    ...
                    if (smaller_box_x <= centroid[0] <= smaller_box_x + smaller_box_width) and (smaller_box_y <= centroid[1] <= smaller_box_y + smaller_box_height):
...
def plot_fresco_assembly(assembly_plan, polygons_data, fragment_names, folder_path, name, block_window:bool = True, pause_duration=0.5):
    ...
    for i in range(len(assembly_plan) - 1):
        ...
        plt.text(current_centroid.x, current_centroid.y, str(i)+"/id:"+str(fragment_names[current_index]), ha='center', va='center')

        ax.fill(next_exterior[:, 0], next_exterior[:, 1], facecolor='green', edgecolor='black')
        plt.text(next_centroid.x, next_centroid.y, str(i+1)+"/id:"+str(fragment_names[next_index]), ha='center', va='center')
...
def plot_fresco_comparison_image(img_path, fresco_polygons_list, ref="fresco", name="", colors=["black", "C0"], visualize=False, save_plot=False):
    ax, fig = plot_fresco_image(img_path, fresco_polygons_list[0], ref=ref, name=name, color=colors[0])
    plot_fresco_image(img_path, fresco_polygons_list[1], ref=ref, name=name, color=colors[1], ax=ax, fig=fig, visualize=visualize, save_plot=save_plot)

def plot_fresco_image(img_path, fresco_polygons, ref="fresco", name="", color="C0", line_width=2.0, plot_grid=True, plot_axis_labels=True, plot_centroids=False, ax=None, fig=None, visualize=False, save_plot=False):
    if name == "":
