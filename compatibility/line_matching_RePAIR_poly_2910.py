...
def line_poligon_intersec_RePair(z_p, z_l, s1, s2, poly0, cfg):
    ...
    # z_p2 = z,   z_l1 = [0,0],
    intersections = []
    piece_j_shape = shapely.polygons(poly0[0])    # CHECK
    piece_j_trans = shapely.transform(piece_j_shape, lambda x: x - [cfg.p_hs, cfg.p_hs] + z_p)
    ...
    for (candidate_xy_start, candidate_xy_end) in zip(s1, s2):
        ...
        # if shapely.is_empty(shapely.intersection(candidate_line_shapely, piece_j_shape)):
        # if shapely.is_empty(shapely.intersection(candidate_line_shapely.buffer(cfg.border_tolerance), piece_j_shape.buffer(cfg.border_tolerance))):
        if shapely.is_empty(shapely.intersection(candidate_line_shapely, piece_j_trans.buffer(cfg.border_tolerance))):
            intersections.append(False)
        else:
            intersections.append(True)
    return intersections
    ...
    #for t in range(len(rot)):
        ...
        for ix in range(m.shape[1]):  # (z_id.shape[0]):
            for iy in range(m.shape[1]):  # (z_id.shape[0]):
                ...
                intersections2 = line_poligon_intersec_RePair([0, 0], z, s21, s22, poly1, cfg)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str, default='repair',
                        help='dataset folder')  # repair, wikiart, manual_lines, architecture,exp_50_lines
    ...
    parser.add_argument('--method', type=str, default='manual', help='method line detection')  # Hough, FLD, Exact, manual
